...
These results were already shown in \textquote{Intersecting restrictions in clutters} by Abdi, Cornuéjols and Lee \cite{restrictions} and \textquote{Deltas, extended odd holes and their blockers} by Abdi and Lee \cite{deltas}.
...
Diese Resultate wurden bereits in \textquote{Intersecting restrictions in clutters} von Abdi, Cornuéjols und Lee \cite{restrictions} und \textquote{Deltas, extended odd holes and their blockers} von Abdi und Lee \cite{deltas} gezeigt.
...
Zudem wurde die Laufzeit des Algorithmus zur Bestimmung eines Delta oder Blocker eines extended odd hole Minoren von $\mathcal{O}(n^4)$ auf $\mathcal{O}(n^3)$ Orakelaufrufe verbessert.
   ...
   In this first chapter, some basic concepts and tools around clutters are introduced, mainly following the introduction of \textquote{Intersecting restrictions in clutters} by Abdi, Cornuéjols and Lee \cite{restrictions}.
...
Thus, $\tau(Q_6) > \nu(Q_6) = 1$, so $Q_6$ is an intersecting clutter.
            ...
            \begin{figure}[b]
                ...
                \begin{tikzpicture}[scale=2.5]
                    ...
                    \draw[red] (0,0) -- node[below, black] {1} (1,0);
                    \draw (0,0) --  (0.4,0.4) -- node[above, black] {5} (1,1);
                    \draw[red] (0,0) -- node[left, black]{4} (0,1);
                    \draw[red] (1,0) -- (0.6, 0.4) -- node[below, black]{6} (0,1);
                    \draw (1,0) -- node[right, black]{3} (1,1);
                    \draw (0,1) -- node[above, black]{2} (1,1);
        ...
        \centering
            ...
            \begin{minipage}{.4\textwidth}
                ...
                \begin{tikzpicture}[scale=1.5]
                    ...
                    \filldraw (3*\x:1) circle (2pt);
                    \filldraw (4*\x:1) circle (2pt);
                    \filldraw (5*\x:1) circle (2pt);
                    \filldraw (6*\x:1) circle (2pt);
                    \filldraw (7*\x:1) circle (2pt);
                    \foreach \i in {0,...,6}
                    {
                        \draw (\i*\x:1) -- (\i*\x+\x:1);
                    }
                    \coordinate (A) at ($ (\x:1) + (0.3,0.3) $ );
                    \coordinate (B) at ($ (3*\x:1) + (-0.3,0.15) $ );
                    \coordinate (C) at ($ (5*\x:1) + (0,-0.4) $ );
                    \draw [rounded corners=5mm] (A)--(B) --(C)--cycle;
                \end{tikzpicture}
                \captionof{figure}{An extended odd hole of dimension 7}
            \end{minipage}
        \end{figure}

        The following lemma is a useful tool to find a delta minor. We will need it in the fourth chapter.
        \begin{lemma}[Abdi, Lee and Pashkovich \cite{ideal}, see also \cite{deltas}, Theorem 5]\label{finddelta}
            Let $\mathcal{C}$ be a clutter over ground set $V$. Let $u,v,w \in V$ be distinct elements and  such that $\{u,v\}$ and $\{u,w\}$ are members of $\mathcal{C}$.
            Let $C \in \mathcal{C}$ such that $\{u,v,w\}\cap C = \{v,w\}$.
            Then $\mathcal{C}$ has a delta minor.
        \end{lemma}
...
Abdi and Lee considered clutters with $\min\{|C|:C\in \mathcal{C}\}=2$ and the graph with vertex set $V$ with the members of cardinality 2 as edges \cite{deltas}. We will consider this graph of the blocker.
   ...
   \begin{algorithm}[H]
       ...
       \Input{Dense clutter $\mathcal{C}$ over ground set $V$ with certificate $w$ inputted via a filter oracle}
   ...
   \begin{proposition}
       The algorithm works correctly in $\mathcal{O}(n^2(\theta+n))$ time.
       If it returns a proper dense minor, a certificate for this minor being dense can be calculated in $\mathcal{O}(n^2(\theta+n))$ time.
   \end{proposition}
